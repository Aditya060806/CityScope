-- 🤖 Simple Enhanced Chatbot Database Setup
-- This script sets up the enhanced chatbot functionality without complex dependencies
...
CREATE TABLE IF NOT EXISTS admin_commands (
    ...
    executed_by UUID, -- No foreign key constraint for now
    executed_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    status command_status DEFAULT 'pending',
    result JSONB DEFAULT '{}',
    error_message TEXT,
    approval_notes TEXT,
    approved_by UUID, -- No foreign key constraint for now
...
CREATE TABLE IF NOT EXISTS language_preferences (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID UNIQUE, -- No foreign key constraint for now
...
DROP POLICY IF EXISTS "Allow all for chatbot_conversations" ON chatbot_conversations;
CREATE POLICY "Allow all for chatbot_conversations" ON chatbot_conversations FOR ALL USING (true);

DROP POLICY IF EXISTS "Allow all for admin_commands" ON admin_commands;
CREATE POLICY "Allow all for admin_commands" ON admin_commands FOR ALL USING (true);

DROP POLICY IF EXISTS "Allow all for voice_commands" ON voice_commands;
CREATE POLICY "Allow all for voice_commands" ON voice_commands FOR ALL USING (true);

DROP POLICY IF EXISTS "Allow all for language_preferences" ON language_preferences;
CREATE POLICY "Allow all for language_preferences" ON language_preferences FOR ALL USING (true);

DROP POLICY IF EXISTS "Allow all for bot_responses" ON bot_responses;
...
DO $$
BEGIN
    RAISE NOTICE '✅ Simple Enhanced Chatbot Database Setup Complete!';
    ...
    RAISE NOTICE '⚠️  Note: Foreign key constraints removed for compatibility';
END $$;
