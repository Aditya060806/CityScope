                ...
                <div className="grid grid-cols-2 gap-3">
                  {Object.entries(CATEGORY_CONFIG).map(([category, config]) => (
                    <Card
                      key={category}
                      className={cn(
                        'p-4 cursor-pointer transition-all duration-200 hover:shadow-civic hover:scale-[1.02]',
                        'border-border/50 hover:border-primary/50 relative overflow-hidden group'
                      )}
                      onClick={() => handleCategorySelect(category as IssueCategory)}
                    >
                      <div className="flex flex-col items-start gap-3 relative z-10 w-full">
                        <div className="w-10 h-10 rounded-lg bg-royal/5 dark:bg-royal/10 flex items-center justify-center text-2xl group-hover:scale-110 transition-transform duration-300">
                          {config.icon}
                        </div>
                        <div className="text-left w-full space-y-1">
                          <p className="font-bold text-lg leading-tight text-slate-900 dark:text-white group-hover:text-primary transition-colors pr-2">
                            {config.label}
                          </p>
