...
export const StatusBadge: React.FC<StatusBadgeProps> = ({ 
  ...
  const mappedStatus = status === 'reported' ? 'pending' : 
                      status === 'verified' ? 'pending' : 
                      status === 'in_progress' ? 'in-progress' : 
                      status;
  
  const config = STATUS_CONFIG[mappedStatus] || STATUS_CONFIG.pending;
  ...
  return (
    <Badge 
      ...
      className={cn(
        ...
        (status === 'reported' || status === 'pending') && 'bg-status-reported/10 text-status-reported',
        status === 'verified' && 'bg-status-verified/10 text-status-verified',
        (status === 'in_progress' || status === 'in-progress') && 'bg-status-progress/10 text-status-progress',
